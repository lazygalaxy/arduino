...
void TMRpcm::play(char *filename, unsigned long seekPoint)
{
	...
	playing = 1;
	bitClear(optionByte, 7); // paused = 0;
	...
	{
		resolution = 10 * ((RESOLUTION_BASE / 2) / SAMPLE_RATE);
	}
	else
	{
		resolution = 10 * (RESOLUTION_BASE / SAMPLE_RATE);
	}
#else
	resolution = 255;
	if (SAMPLE_RATE < 9000)
...
ISR(TIMER2_OVF_vect)
{
	...
	if (volMod < 0)
	{
		*OCRnB[tt] = buffer[whichBuff][buffCount] >> (volMod * -1);
	}
	else
	{
		*OCRnB[tt] = buffer[whichBuff][buffCount] << volMod;
	}
	++buffCount;
...
ISR(TIMER1_COMPB_vect)
{
	...
	if (recording > 1)
	{
		if (volMod < 0)
		{
			*OCRnA[tt] = ADCH >> (volMod * -1);
		}
		else
		{
			*OCRnA[tt] = ADCH << volMod;
		}
	}
	buffCount++;
	if (buffCount >= buffSize)
...
void TMRpcm::play(char *filename, unsigned long seekPoint, boolean which)
{
	...
	if ((!playing && !playing2) || (playing && !playing2 && !which) || (playing2 && !playing && which))
	{
		...
		if (seekPoint > 0)
		{
			seekPoint = SAMPLE_RATE * seekPoint;
		}
		if (!which)
		{
#if !defined(SDFAT)
...
void TMRpcm::ramp(boolean wBuff)
{
	...
	else
	{
		...
		if (volMod >= 0)
		{
			mod = *OCRnB[tt2] >> volMod;
		}
		else
		{
			mod = *OCRnB[tt2] << (volMod * -1);
		}
...
#if !defined(SDFAT)
			xFile.seek(40);
			unsigned int siz = xFile.read();
			siz = (xFile.read() << 8 | siz) + 2;
...
#endif
		listLen = xFile.read() << 7 | listLen;
		listLen = xFile.read() | listLen;
		tagNames[0] = "TPE1";
		tagNames[1] = "TIT2";
		tagNames[2] = "TALB";
#if !defined(SDFAT)
...
//**This section jumps to the next tag position if the requested tag wasn't found
	...
	if (infoType == 0)
	{
		if (!found)
		{
			xFile.seekSet(xFile.curPosition() + len);
		}
		while (xFile.peek() == 0)
		{
			xFile.read();
